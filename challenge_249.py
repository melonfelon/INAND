...
def stock_market(prices):
    ...
    for i in price_list:
        if i < low:
            low = i
            position = price_list.index(i)
    for x in range(position, len(price_list)):
        if price_list[x] > high and x != (position +1):
            high = price_list[x]

    print (low, high)

stock_market([9.20, 8.03, 10.02, 8.08, 8.14, 8.10, 8.31, 8.28, 8.35,
